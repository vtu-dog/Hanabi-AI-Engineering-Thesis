...
Strategia agenta Erratic polega na~wykonywaniu serii przypadkowych ruchów.

\subsection*{Schemat działania}

\begin{itemize}
	\item Wylosuj jedną legalnych akcji (zagraj kartę, odrzuć kartę, udziel podpowiedzi).
...
Agent wykonujący wyłącznie losowe ruchy jest pierwszym, który nie~oszukuje. Niestety, jak można było przewidywać, przegrywa on~większość gier. Jest to~spowodowane brakiem mechanizmu, który powstrzymywałby go~przed zagrywaniem kart, które nie~są~bezpieczne. Szansa na~udzielanie informacji, które w~tym przypadku są~bezużyteczne, nie~zaburza wyników, gdyż wykorzystanie żetonu podpowiedzi nie~prowadzi do~eskalacji progresu gry -~jeżeli nie~dobrano ostatniej karty z~talii.

\section{StoppedClock}

Agent StoppedClock rozszerza strategię agenta Erratic: wykonuje losowe ruchy, które nie~mogą doprowadzić do~utraty żetonów życia. Jego postępowanie, choć bardzo chaotyczne, czasem prowadzi do~wysokich punktacji, zgodnie z~maksymą: ``nawet zepsuty zegar dwa razy na~dobę pokazuje właściwą godzinę''.
...
\begin{itemize}
	...
	Jedna z~miar używanych w~funkcjach oceniających przydatność ruchów: jeżeli uzyskana przez daną akcję nota jest niższa od~pułapu, jest ona dyskwalifikowana jako niewystarczająco pomocna. 

	\item \textbf{Heurystycznie wartościowa akcja}:
	
	Niektóre akcje, takie jak bezpieczne zagranie, są~w~oczywisty sposób korzystne dla postępu gry. Niestety, przypisanie części ruchów jednoznacznej wartości liczbowej, wskazującej na jej przydatność w~kontekście danego momentu rozgrywki, jest trudnym zadaniem, gdyż pełna analiza możliwych następstw akcji wymagałaby wysokiego nakładu mocy obliczeniowej. Aby uniknąć tego problemu, można zastosować funkcje heurystyczne, które przybliżają wartości poszukiwanych~ocen. Akcja, która uzyska najwyższą notę, a~także będzie wyższa od~zadanego pułapu jakościowego, jest nazywana heurystycznie optymalną.
	
	\item \textbf{Najstarsza (najmłodsza) karta}:
	
	Karta, która znajduje~się w~ręce danego gracza od~największej (najmniejszej) ilości tur. W~razie konfliktu za~najstarszą (najmłodszą) kartę uznawana jest ta~wysunięta najbardziej na~lewo.
...
\begin{itemize}
	...
	Heurystycznie wartościowa podpowiedź, będąca obietnicą lub komunikatem, ignorująca pułap jakościowy heurystyki.
...
\begin{enumerate}
	...
	\item Jeżeli gracz otrzyma podpowiedź wskazującą na~numer, którego poprawne zagranie byłoby niemożliwe w~turze podpowiadającego gracza, podpowiedź jest komunikatem.
	\item Podpowiedź jest obietnicą.
\end{enumerate}

Podobnie jak w~przypadku agenta Distrustful, program składa~się z~dziesięciu modułów, które są~rozważane po~kolei w~jednej z~czterech konfiguracji. Każdy moduł, poza ostatnim, może zawieść. Ich kolejność zależy od~ilości posiadanych żetonów podpowiedzi:
...
\begin{enumerate}
	...
	Jeżeli istnieje heurystycznie wartościowa podpowiedź lub obietnica, która umożliwi innemu graczowi bezpieczne lub inferowalnie bezpieczne zagranie, udziel jej.
	...
	Jeżeli istnieje podpowiedź ratunkowa ujawniająca kartę krytyczną, którą można wykonać w~trybie komunikatu, wykonaj ją.
...
Podpowiadanie kart w~trybie obietnicy rzadko przebiega w~bezproblemowy sposób: karty tworzące częściowy porządek w~ręce gracza to~rzadkie zjawisko. Jest to~powód, dla którego funkcje heurystyczne agresywnie zaniżają noty obietnic, których pierwszy element jest niepoprawny, lecz relaksują kary dla tych podpowiedzi, które zaczynają~się w~sposób umożliwiający co~najmniej jedno bezpieczne zagranie.
...
Strategia agenta nie~jest~przystosowana do~rozgrywek dwuosobowych, gdyż udzielenie dowolnej podpowiedzi wetuje kartę, która miała szansę być właśnie zagrana. Prowadzi to~do konieczności omijania systemu obietnic. Można to~osiągnąć wyłącznie poprzez wykonywanie jednej z~pozostałych akcji, co~jest trudnym zadaniem, gdyż wiąże~się to~z koniecznością dobrej znajomości własnej ręki.
...
Agent Trustful nie tylko potrafi przekroczyć próg 20~punktów, ale i~zdarza mu~się~wygrać: zastosowanie protokołu implicytnej komunikacji pozwoliło wynieść osiągi sztucznej inteligencji na~nowy poziom. Niestety, strategia programu nie~jest przystosowana do~rozgrywek dwuosobowych, co~objawia~się wysoką niestabilnością algorytmu. Wartym zaobserwowania faktem, który można odczytać z~wykresu, jest wykładniczy wzrost trudności uzyskiwania kolejnych punktów, który występuje od~pewnego momentu rozgrywki.
...
Proces gaussowski to proces stochastyczny, który pozwala na~określenie łącznego rozkładu prawdopodobieństwa (a~także jego niepewności) wybranej rodziny nieparametrycznych funkcji $\mathbb{F}$. Polega on~na~konstrukcji prawdopodobieństw $\textit{a priori}$, które można aktualizować poprzez kolejne obserwacje, co~tworzy system przekonań o~kształcie badanych funkcji.

\begin{figure}[H]
	\centering
	\captionsetup{format=hang}
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{Gauss.png}
	\caption[Caption]{Wizualizacja procesu gaussowskiego. Zielonymi plusami zaznaczono obserwacje, niebieskie krzywe to~poprzednie przekonania $\textit{a posteriori}$, zaś czerwona krzywa to~ich średnia. Zacieniona część wykresu wyznacza niepewność modelu. Żródło: $\cite{Gauss}$}
	\label{fig:Gauss}
\end{figure}

...
class LastCard(BoxLayout):
    def __init__(self, **kwargs):
        super(LastCard, self).__init__(**kwargs)

    def set_card(self, game):
        self.hand = Hand(
            [game.hands[0][0]],
            full_info=False, is_tintable=False
        )
        self.hand.label.text = 'Last action'
        self.add_widget(self.hand)

    def update(self):
        self.hand.cards[0].full_info = True
        self.hand.cards[0].real_rank = hanabi_game.player_choice.real_rank
        self.hand.cards[0].real_suit = hanabi_game.player_choice.real_suit
        self.hand.update(last_card=True)
...
class HanabiGame(BoxLayout):
    ...
    def prepare_game(self, other_players, hint):
        ...
        self.last_card.set_card(self.game)
